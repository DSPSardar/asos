...
const welcomeVoiceSvc = require('./welcomeVoice.service');
...
const uploadWelcomeVoice = async (req, res, next) => {
  try {
    if (!req.file) return error(res, 'No audio file uploaded', 400);
    const config = await welcomeVoiceSvc.uploadWelcomeVoice(req.tenantId, req.file.buffer, req.file.originalname);
    return success(res, config, 'Welcome voice note uploaded');
  } catch (e) { next(e); }
};

module.exports = { get, update, test, usage, uploadWelcomeVoice };
