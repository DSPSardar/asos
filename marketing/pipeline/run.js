...
const { PATHS, PIPELINE_STEPS } = require('./config');
const { runAgentStep } = require('./agentRunner');
...
async function runStep({ step }, previousOutput, anthropic) {
  const json = await runAgentStep(step, previousOutput, anthropic);
